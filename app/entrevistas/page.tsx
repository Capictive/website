...
const CustomInterviewRenderer = ({ content }: { content: string }) => {
  ...
  const evalMatch = content.match(
    /## Evaluación general de coherencia([\s\S]*?)(?:> \*\*Nota|$)/,
  );
  ...
  const coherenceLevelMatch = evaluationText.match(
    /\*\*Nivel de coherencia observado:\*\*\s*(.*)/,
  );
    ...
    <div className="space-y-10 font-body text-subtitle">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {keyPoints.map((point, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex gap-3 items-start p-3 rounded-lg bg-gray-50 hover:bg-button-background-secondary/20 transition-colors"
                >
      ...
      {comparisons.length > 0 && (
        <section>
          <div className="mb-8 border-l-4 border-button-background-primary pl-4">
            <h2 className="font-title text-3xl font-bold text-subtitle">
              Comparación con el Plan de Gobierno
            </h2>
            <p className="text-subtitle/70 mt-1">
              Análisis de coherencia entre lo dicho y lo escrito.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {comparisons.map((comp, i) => {
              ...
              const saidMatch = comp.body.match(
                /- \*\*Lo dicho en la entrevista:\*\*([\s\S]*?)(?=- \*\*Lo establecido)/,
              );
              const planMatch = comp.body.match(
                /- \*\*Lo establecido en el plan:\*\*([\s\S]*?)(?=\*\*Evaluación:)/,
              );
              const evalResultMatch = comp.body.match(
                /\*\*Evaluación:\*\*([\s\S]*?)(?=\*|$)/,
              ); // Capture up to italized explanation start or end
              ...
              return (
                <div
                  key={i}
                  className="bg-white rounded-xl overflow-hidden border border-subtitle/10 shadow-md"
                >
                  {/* Header */}
                    ...
                    {result && (
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold border ${
                          result.includes("ALTO") ||
                          result.includes("Coherente") ||
                          result.includes("✔️")
                            ? "bg-green-100 text-green-700 border-green-200"
                            : result.includes("Parcial") ||
                                result.includes("🟡")
      ...
      {(coherenceLevel || conclusion) && (
        <section className="bg-gradient-to-br from-button-background-secondary/30 to-white p-6 rounded-xl border border-button-background-primary/20 shadow-lg">
          <h2 className="font-title text-2xl font-bold text-subtitle mb-4">
            Evaluación General
          </h2>

          {coherenceLevel && (
            <div className="mb-4 inline-flex items-center gap-3 bg-white px-4 py-2 rounded-lg shadow-sm border border-subtitle/10">
              <span className="font-bold text-sm text-subtitle/70">
                Nivel de Coherencia:
              </span>
              <span className="font-title font-extrabold text-xl text-button-background-primary">
                {coherenceLevel}
              </span>
            </div>
          )}
      ...
      {keyPoints.length === 0 && comparisons.length === 0 && (
        <div className="p-4 bg-red-50 text-red-700 rounded-lg">
          <p>
            No se pudo analizar el formato estructurado. Mostrando texto plano:
          </p>
...
function EntrevistasContent() {
  ...
  const partyMetadata = useMemo(() => {
    ...
    return (
      PARTIES.find((p) => p.name.toLowerCase() === partidoName.toLowerCase()) ||
  ...
  useEffect(() => {
    if (partidoName) {
      ...
      setError(null);
      fetch(
        `https://api.capictive.app/entrevistas?partido=${encodeURIComponent(partidoName)}`,
      )
        .then((res) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        <Image
          ...
          height={150}
        />
        <p className="font-body text-subtitle text-lg">
          ⚠️ No se ha seleccionado un partido.
        </p>
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="font-body text-subtitle mt-4">
        ...
        >
          ...
          <Link href="/partidos" className="block mt-2 underline">
            Volver a intentar
          </Link>
        </div>
      ) : selectedEntrevista ? (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
          ...
          <button
            onClick={() =>
              router.push(
                `/entrevistas?partido=${encodeURIComponent(partidoName)}`,
              )
            }
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
              <div>
                <h1 className="font-title text-subtitle text-3xl md:text-5xl font-extrabold">
                  Entrevistas:{" "}
                  <span className="text-button-background-primary">
                    {partyMetadata?.name || partidoName}
                  </span>
                </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {entrevistas.map((e) => (
              <div
                ...
                onClick={() =>
                  router.push(
                    `/entrevistas?partido=${encodeURIComponent(partidoName)}&expediente=${e.entrevista_id}`,
                  )
                }
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <h3 className="font-title text-lg font-bold text-subtitle line-clamp-3 mb-2 group-hover:text-button-background-primary transition-colors">
                    {e.hook_ia ||
                      e.titulo_ia?.substring(0, 80) ||
                      "Entrevista sin título"}
                  </h3>
