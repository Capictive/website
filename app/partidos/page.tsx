...
async function fetchPartyScandals(
  partyName: string,
): Promise<Escandalo[] | null> {
  try {
    const encodedName = encodeURIComponent(partyName);
    const res = await fetch(
      `https://controver.capictive.app?partido=${encodedName}`,
    );
    if (!res.ok) return null;
...
export default function PartidosPage() {
  ...
  const [scandalsState, setScandalsState] = useState<{
  ...
  const filtered = useMemo(() => {
    ...
    return PARTIES.filter(
      (p) =>
        p.name.toLowerCase().includes(q) && !blockedParties.includes(p.name),
    );
  ...
  useEffect(() => {
    if (selected) {
      ...
      fetchPartyScandals(selected.name).then((data) =>
        setScandalsState({ scandals: data, loading: false }),
      );
    } else {
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                ...
                <div className="flex-1 space-y-3 text-center md:text-left">
                  ...
                  <div className="tour-documentos flex flex-wrap gap-3 font-body justify-center md:justify-start">
                    <a
                      href={`https://files.capictive.app/Partidos%20Politicos/${encodeURIComponent(detailState.detail.partido)}/PLAN%20RESUMEN.pdf`}
                      target="_blank"
                    ...
                    <a
                      href={`https://files.capictive.app/Partidos%20Politicos/${encodeURIComponent(detailState.detail.partido)}/PLAN%20GOBIERNO.pdf`}
                      target="_blank"
                    ...
                    <a
                      href={`https://files.capictive.app/Partidos%20Politicos/${encodeURIComponent(detailState.detail.partido)}/HOJA%20DE%20CANDIDATOS.pdf`}
                      target="_blank"
                      className="btn-secondary text-sm"
                    >
                      👥 Hoja de Candidatos
                  ...
                  <>
                    {scandalsState.loading ? (
                      <div className="text-center py-12">
                        <div className="inline-block animate-spin text-4xl mb-2">
                          🔄
                        </div>
                        <p className="font-body text-subtitle">
                          Investigando escándalos...
                        </p>
                      </div>
                    ) : scandalsState.scandals &&
                      scandalsState.scandals.length > 0 ? (
                      <>
                        {/* Mobile Swipeable */}
                        <div className="md:hidden">
                          <EscandalosSwipeable
                        ...
                        <div className="hidden md:block">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              <EscandaloCard
                                escandalo={
                                  scandalsState.scandals[currentEscandaloIndex]
                                }
                              />
                            </div>
                            <button
                              ...
                              onClick={() =>
                                setCurrentEscandaloIndex((i) =>
                                  Math.min(
                                    scandalsState.scandals!.length - 1,
                                    i + 1,
                                  ),
                                )
                              }
                              disabled={
                                currentEscandaloIndex ===
                                scandalsState.scandals.length - 1
                              }
                            >
                              ›
                          ...
                          <p className="text-center font-body text-sm mt-3">
                            {currentEscandaloIndex + 1} de{" "}
                            {scandalsState.scandals.length}
                          </p>
                        </div>
                      </>
                    ) : (
                      <div className="text-center py-8 border rounded-xl bg-green-50">
                        <span className="text-4xl mb-2 block">✅</span>
                        <p className="font-body text-subtitle">
                          No se han registrado escándalos mayores o no hay
          ...
          <div className="bg-primary-background rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scaleIn border-2 border-subtitle">
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-16 h-16 bg-button-background-secondary rounded-full flex items-center justify-center mb-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-8 w-8 text-button-background-primary"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
  ...
  return (
    <div className="border rounded-xl p-4 md:p-5 space-y-4 shadow-lg bg-white">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h4 className="font-title text-subtitle text-lg md:text-xl font-bold text-red-700">
            {escandalo.titulo}
          </h4>
        </div>
      </div>
      ...
      <div className="bg-red-50/50 p-4 rounded-lg border-l-4 border-red-500">
        <p className="font-body text-sm text-subtitle leading-relaxed">
          {escandalo.información}
        </p>
      </div>
      ...
      {escandalo.involucrados && escandalo.involucrados.length > 0 && (
        <div className="space-y-2">
          <p className="font-body text-xs uppercase font-bold text-subtitle/70">
            Involucrados:
          </p>
          <div className="flex flex-wrap gap-2">
            {escandalo.involucrados.map((persona, i) => (
              <span
                key={i}
                className="px-2 py-1 bg-gray-100 rounded text-xs font-body font-medium text-gray-700 border border-gray-200"
              >
                👤 {persona}
              </span>
            ))}
          ...
          <div className="mt-2 space-y-2 p-2 bg-gray-50 rounded text-xs break-all">
            {escandalo.fuentes.map((fuente, i) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block text-subtitle hover:underline hover:text-button-background-primary"
              >
                Link {i + 1}
              </a>
            ))}
    ...
    <div className="space-y-3">
      ...
      <div className="flex justify-center items-center gap-3">
        ...
        <button
          className="btn-secondary text-lg px-3 py-1 disabled:opacity-40"
          onClick={() =>
            setCurrentIndex((i) => Math.min(escandalos.length - 1, i + 1))
          }
