...
export default function Nav() {
  ...
  const item = (href: string, label: string, tourClass?: string) => {
    const selected = pathname === href;
    const cls = selected ? "nav-selected" : "nav-unselected";
    return (
      <Link
        href={href}
        className={`${cls} ${tourClass ?? ""} text-center font-bold font-body`}
      >
        {label}
      </Link>
    );
  ...
  return (
    <div>
      <div className="text-center grid grid-cols-2 sm:grid-cols-4 gap-2 font-bold font-body">
        {item("/", "Inicio", "tour-nav-inicio")}
        {item("/partidos", "Partidos Politicos", "tour-nav-partidos")}
        {item("/comparar", "Comparar", "tour-nav-comparar")}
        {item("/candidatos", "Candidatos", "tour-nav-candidatos")}
      </div>
